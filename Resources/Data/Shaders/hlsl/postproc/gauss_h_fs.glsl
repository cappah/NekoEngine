...
void main()
{
	o_FragColor = texture(GET_TEX_2D(u_texture0), vec2(gl_FragCoord)/1024.0) * weight[0];

	for (int i = 1; i < 3; i++) 
	{
		o_FragColor += texture(GET_TEX_2D(u_texture0), (vec2(gl_FragCoord) + vec2(offset[i], 0.0)) / 1024.0) * weight[i];
		o_FragColor += texture(GET_TEX_2D(u_texture0), (vec2(gl_FragCoord) - vec2(offset[i], 0.0)) / 1024.0) * weight[i];
	}
}
