...
void main()
{
	...
	sample0 = texture(GET_TEX_2D(PreviousImage), vec2(v_uv.x - blurStep, v_uv.y - blurStep)).rgb; 
	sample1 = texture(GET_TEX_2D(PreviousImage), vec2(v_uv.x + blurStep, v_uv.y + blurStep)).rgb;
	sample2 = texture(GET_TEX_2D(PreviousImage), vec2(v_uv.x + blurStep, v_uv.y - blurStep)).rgb;
	sample3 = texture(GET_TEX_2D(PreviousImage), vec2(v_uv.x - blurStep, v_uv.y + blurStep)).rgb;
