...
//
// Changes:
// - Modified load() to support the VFS implementation
// - Fixed data types for 64-bit *NIX systems (*NIX uses LP64 instead of Windows's LLP64)
// - Modified to use GLEW; the library wasn't able to find glCompressedTexImage* on *NIX
// - Replaced GLEW with GLAD
// - Added CDDSImage::loadFromMemory
//
///////////////////////////////////////////////////////////////////////////////
//
// Description:
...
bool CDDSImage::load(string filename, bool flipImage)
{
	...
	// read in DDS header
	DDS_HEADER ddsh;
	if(fread(&ddsh, sizeof(DDS_HEADER), 1, fp) != 1)
	{
		fclose(fp);
		return false;
	}
...
bool CDDSImage::loadFromMemory(const uint8_t *mem, size_t size, bool flipImage)
{
	...
	// read in DDS header
	DDS_HEADER ddsh;
	memcpy(&ddsh, ptr, sizeof(DDS_HEADER));
